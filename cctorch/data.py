...
class CCIterableDataset(IterableDataset):
    ...
    #     def read_file(fd):
    #         # Profile disk I/O
    #         return read_data(fd, self.data_path1, self.data_format1, mode=self.mode)
    
    #     with ThreadPoolExecutor(max_workers=2) as executor:
    #         futures = [executor.submit(read_file, fd) for fd in data_list]
    ...
    #                 concatenated_data = torch.cat(concat_data, dim=-1).to(self.device)
    ...
    def sample_ambient_noise(self, data_list):
        ...
        #        yield processed_data
            ...
            for i, j in block_index:
                ...
                if self.config.stack_channel == 1:
